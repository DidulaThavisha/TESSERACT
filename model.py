...
class ResNet(nn.Module):
    ...
    def __init__(self, name='vgg19', num_classes=2):
        ...
        if name == 'vgg19':
            ...
            self.encoder = torch.hub.load('mateuszbuda/brain-segmentation-pytorch', 'unet', in_channels=1, out_channels=1, init_features=32, pretrained=True)
            self.encoder.conv1 = nn.Conv2d(1, 64, kernel_size=(1, 1), stride=(1, 1), padding=(1, 1))
            #self.encoder.features[0] = nn.Conv2d(1, 64, kernel_size=(3, 3), stride=False, padding=(1, 1))
